...
def calculate_indicators_single_ticker(close_interval: pd.DataFrame, rsi_window: int, lowess_fraction: int) -> Tuple[np.array, np.array, np.array]:
    ...
    return rsi, first_derivative, second_derivative

def calculate_indicators_multiple_tickers(close_interval: pd.DataFrame, rsi_window: int, lowess_fraction: int) -> Tuple[np.array, np.array, np.array]:
    ...
    #calculate rsi
    rsi = vbt.RSI.run(close_interval, rsi_window).rsi.to_numpy()
...
def custom_indicator(close_interval: pd.DataFrame, lowess_fraction: int, velocity_up: float, velocity_down: float,
    ...
    tickers = list(close_interval.columns)
    if len(tickers) == 1:
        rsi, first_derivative, second_derivative = calculate_indicators_single_ticker(close_interval, rsi_window, lowess_fraction)
    elif len(tickers) > 1:
        rsi, first_derivative, second_derivative = calculate_indicators_multiple_tickers(close_interval, rsi_window, lowess_fraction)
